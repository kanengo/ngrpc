...
func markSuccessWithDuration(b circuitbreaker.Breaker, count int, sleep time.Duration) {
	for i := 0; i < count; i++ {
		b.MarkSuccess()
		time.Sleep(sleep)
	}
...
func markFailedWithDuration(b circuitbreaker.Breaker, count int, sleep time.Duration) {
	for i := 0; i < count; i++ {
		b.MarkFailed()
		time.Sleep(sleep)
	}
...
func markSuccess(b circuitbreaker.Breaker, count int) {
	for i := 0; i < count; i++ {
		b.MarkSuccess()
	}
}

func markFailed(b circuitbreaker.Breaker, count int) {
	for i := 0; i < count; i++ {
		b.MarkFailed()
	}
}

func testSREClose(t *testing.T, b circuitbreaker.Breaker) {
	markSuccess(b, 80)
...
func testSREOpen(t *testing.T, b circuitbreaker.Breaker) {
	markSuccess(b, 100)
	assert.Equal(t, b.Allow(), nil)
...
func testSREHalfOpen(t *testing.T, b circuitbreaker.Breaker) {
	// failback
	assert.Equal(t, b.Allow(), nil)
